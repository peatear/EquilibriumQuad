...
public class btConManager
{
    ...
    public async Task<uint> SendCommand(float[] command)
    {
        uint sentCommandSize = 0;
        if (dataWriter != null)
        {
            
            dataWriter.WriteInt16(Convert.ToInt16(command[0]));
            dataWriter.WriteInt16(Convert.ToInt16(command[1]));
            dataWriter.WriteInt16(Convert.ToInt16(command[2]));
            dataWriter.WriteInt16(Convert.ToInt16(command[3]));
